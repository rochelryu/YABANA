...
export default class Ikea {
    ...
    private  _table: any;
    private  _canape: any;
    private  _meubleTV: any;
    private  _biblio: any;
    private  _armoir: any;
    private  _lit: any;
    private  _buffet: any;
    private  _cafe: any;
    private  _dessert: any;
    private  _jardin: any;
    private  _commode: any;



    //Cuisine
    private  _meubleCuisine: any;
    private  _cuisineComplete: any;
    private  _electromenager: any;
    private  _planDeTravail: any;
    private  _AmenagementInterieur: any;
    private  _rangementInterieur: any;
    private  _eclairageInt: any;




    private  _total: any[];
    private  _totalCuisine: any[];
    private details: any = "https://www.ikea.com/fr/fr/p/";
    constructor(){
...
*/
    ...
    private requestAsyncForMore = (url:string)=>{
    ...
    private requestAsyncForMinus = (url:string)=>{
    ...
    async ikeaDetails(keyword:string){
    ...
    private async filterWithSearch(attribut:any){

            await Produit.findOne({ident:attribut.id}).then(async (result)=>{
                if(result !== null){
                    return result;
                }
                else{
                    ...
                    const product:any = new Produit(tcheck);
                    await product.save().then((resss:any)=>{
                        console.log("save !!")
                        return resss
                    }).catch((errr:Error)=>console.log("pas enregistré", errr) )
                }
            }).catch(errors=>console.log("req Find One inopérante parce que:", errors))
        }
    public ikeaSearch(keyword:string, min:number,max:number) {
        ...
        return new Promise(async next=>{
            await Produit.find({ $or: [ {"type_name":{'$regex': search}}, {"catalog_name":{'$regex': search}} ] }).then(async ress=>{//.sort({$natural:-1})
                if(ress.length >= 6){
                    ...
                    try{
                        await fetch(urls)
                            ...
                            .then(async (res:any) => {
                                ...
                                let focuss = await ell.map((value,index,array) => this.filterWithSearch(value.attributes))
                                await Produit.find({ $or: [ {"type_name":{'$regex': search}}, {"catalog_name":{'$regex': search}} ] })
                                    .then(resultat=>{
                                        next(resultat)
     ...
     public home() {
        return new Promise(async next=>{
            await SousCategorie.find({Categorie:1}).sort({name:1}).then(async res=>{//.sort({$natural:-1})
                if(res.length !== 0){
                    ...
                    let ell:any[] = [];
                    try{
                        let data:any = await Promise.all(this._total.map((value,index)=>this.requestAsync(value[0])));
                        for(let i=0 ; i<data.length; i++){
                            ...
                            if(tcheck.ident === 5 || tcheck.ident === 12){
                                tcheck.products = data[i].categoryProducts[1].products.map((value:any,index:number)=> {
                                ...
                                tcheck.products = data[i].categoryProducts[0].products.map((value:any,index:number)=> {
                                    return {name:value.attributes.name, ident:value.attributes.id,profil: value.attributes.main_image_url, price:((parseInt(value.attributes.price_numeral, 10)*655)+ (parseInt(value.attributes.price_numeral, 10)*655*this._taux)).toString(),catalog_name:value.attributes.catalog_name,type_name:value.attributes.type_name,material_name:value.attributes.material_name,desc:value.attributes.benefit_list,is_new:value.attributes.is_new_product,color_name:value.attributes.color_name,url:value.attributes.pip_url, rating_value:value.attributes.rating_value,valid_design_text:value.attributes.valid_design_text}
                                });
                            }
                            ell.push(tcheck);
                        ...
                        ell.map(async (value,index)=>{
                            const sousCategorieItem:any = new SousCategorie(value);
                            await sousCategorieItem.save().then((ress:any)=>console.log("enregistré avec succès")).catch((errr:Error)=>console.log("pas enregistré", errr) )
                        });
                        next(ell)
                    }
                    catch (e) {
    ...
    public homeCuisine() {
        return new Promise(async next=>{
            await SousCategorie.find({Categorie:3}).sort({name:1}).then(async res=>{//.sort({$natural:-1})
                if(res.length !== 0){
                    console.log("il y a res", res.length);
                    next(res)
                }
                    ...
                    let ell:any[] = [];
                    try{
                        let data:any = await Promise.all(this._totalCuisine.map((value,index)=>this.requestAsync(value[0])));
                        for(let i=0 ; i<data.length; i++){
                            let tcheck:any = {};
                            tcheck.numberTotal = parseInt(data[i].categoryOverview[0].categoryTree.productCount, 10);
                            tcheck.name = data[i].categoryOverview[0].categoryTree.displayName;
    ...
    public async getProductByCategorie(key:number) {
        let element:any[] = [];
        for(let j = 0; j < this._total.length; j++){
            if(this._total[j][1] === key) element = this._total[j]
        }
        return new Promise(async next=>{
            await Produit.find({souscategorie:element[1]}).then(async res=>{
                if(res.length > 0){
                    console.log("trouvé produit de cette categorie", res.length)
                    let datas = await SousCategorie.find({Categorie:element[2]}).sort({name:1})
                    next({data:res, datas:datas, concerne:element[1]})
                }
                else {
                    try{
                        let datas = await SousCategorie.find({Categorie:element[2]}).sort({name:1})
                        let data:any = await this.requestAsyncForMore(element[0])
                        let ell:any[] = [];
                        ...
                        ell.map(async (value,index)=>{
                            const ProduitItem:any = new Produit(value);
                            await ProduitItem.save().then((ress:any)=>console.log("enregistré avec succès")).catch((errr:Error)=>console.log("pas enregistré") )
                        });
    ...
    public async getProductByCategorieC(key:number) {
        let element:any[] = [];
        for(let j = 0; j < this._totalCuisine.length; j++){
            if(this._totalCuisine[j][1] === key) element = this._totalCuisine[j]
        }
        return new Promise(async next=>{
            await Produit.find({souscategorie:element[1]}).then(async res=>{
                if(res.length > 0){
